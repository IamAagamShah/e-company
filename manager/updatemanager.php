<?php
    ...
    if(isset($_POST['submit'])){
        if(strlen($_POST['empname']) != 0){
            ...
            $ab3_query = "SELECT s_assignid FROM shifttab WHERE s_assignid = '".$_SESSION['dash_Id']."';";
            $ab3_run = mysqli_query($conn, $ab3_query);
            $sell3_row = mysqli_fetch_array($ab3_run);
            if($sell3_row['s_assignid'] == $_SESSION['dash_Id']){
                $sell3_query = "UPDATE shifttab SET s_assignname = '".$UserName."' WHERE s_assignid = '".$_SESSION['dash_Id']."';";
                $sell3_run = mysqli_query($conn, $sell3_query);
            }

            $ab4_query = "SELECT l_approveid FROM leavetab WHERE l_approveid = '".$_SESSION['dash_Id']."';";
            $ab4_run = mysqli_query($conn, $ab4_query);
            $sell4_row = mysqli_fetch_array($ab4_run);
            if($sell4_row['l_approveid'] == $_SESSION['dash_Id']){
                $sell4_query = "UPDATE leavetab SET l_approvename = '".$UserName."' WHERE l_approveid = '".$_SESSION['dash_Id']."';";
                $sell4_run = mysqli_query($conn, $sell4_query);
            }
            $ab5_query = "SELECT l_applyid FROM leavetab WHERE l_applyid = '".$_SESSION['dash_Id']."';";
            $ab5_run = mysqli_query($conn, $ab5_query);
            $sell5_row = mysqli_fetch_array($ab5_run);
            if($sell5_row['l_applyid'] == $_SESSION['dash_Id']){
                $sell5_query = "UPDATE leavetab SET l_applyname = '".$UserName."' WHERE l_applyid = '".$_SESSION['dash_Id']."';";
                $sell5_run = mysqli_query($conn, $sell5_query);
            }
